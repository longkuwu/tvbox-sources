...
def detail(id):
    ...
    # 选集解析
    # 实际结构: 播放源名称在 data-dropdown-value 中，集数在 module-play-list-link 中
    play_from_list = []
    play_url_list = []
    ...
    # 提取所有集数面板，每个面板对应一个播放源
    panels = re.split(r'<div[^>]*class="[^"]*module-list[^"]*"[^>]*>', html)

    for idx, panel in enumerate(panels[1:], start=0):
        # 获取对应的播放源名称
        if idx < len(source_names):
            source_name = source_names[idx]
        else:
            source_name = "线路" + str(idx + 1)

        # 提取集数链接: <a class="module-play-list-link" href="/iyfplay/..." title="..."><span>集名</span></a>
        eps = re.findall(
            r'<a[^>]+href="(/iyfplay/(\d+)-(\d+)-(\d+)/)"[^>]*>\s*<span>([^<]*)</span>\s*</a>',
            panel
        )
        if eps:
            ep_list = []
